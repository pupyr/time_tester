...
double test_time(void(*func)(long long), long long N){
    double sum = 0;
...
std::vector<std::vector<std::string>> init_vect(int size, int start, int N, int step){
    ...
    for(int i=0; i<1+size; i++){
        ...
        for(int j=0; j<=N; j++){
            if(i>0) mass_i.push_back("");
            else if(j>0) mass_i.push_back(get_pow(start+(j-1)*step));
        }
...
void func_test(long long start, int N, long long step, std::vector<void(*)(long long)> funcs){
    ...
    for(int f=0; f<funcs.size(); f++){
        ...
        for(int i=0; i<N; i++){
            ...
            double res = test_time(funcs[f], start+i*step);
